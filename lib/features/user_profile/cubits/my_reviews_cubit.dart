...
class MyReviewsCubit extends Cubit<PagedState<Review>> {
  MyReviewsCubit(this._repo) : super(const PagedState<Review>());
  ...
  Future<void> loadInitial() async {
    ...
    try {
      ...
      final items = await _repo.getMyReviews(
        accessToken: token,
        limit: _limit,
        skip: 0,
      );
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      final next = await _repo.getMyReviews(
        accessToken: token,
        limit: _limit,
        skip: state.skip,
      );
