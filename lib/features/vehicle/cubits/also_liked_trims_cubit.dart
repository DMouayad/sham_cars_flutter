...
class AlsoLikedTrimsCubit extends Cubit<AlsoLikedTrimsState> {
  AlsoLikedTrimsCubit(this._repo) : super(const AlsoLikedTrimsState());
  ...
  Future<void> load(int trimId) async {
    ...
    try {
      final items = await _repo.getAlsoLikedTrims(trimId, take: _take, skip: 0);
      emit(
        state.copyWith(
  ...
  Future<void> loadMore() async {
    ...
    try {
      final next = await _repo.getAlsoLikedTrims(
        _trimId!,
        take: _take,
        skip: state.skip,
      );
